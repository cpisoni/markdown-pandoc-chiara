...
- Structured Query Language (SQL) code are products of the project. 
    - All updateable datasets are acquired from the original data source (for
...
will be created that defines the relational structure of the database. 
A
...
# Background {-}
![Figure 1. Area Map](img/rio-grande-basin.png){ width=50% }

[Map source: @Bartolino_1979]
...
This dataset has `r nrow(usgs)` sampling sites.
...
1. All original raw data files and data source processing programs will be
versioned over time and maintained in a date-stamped file structure with
text files documenting the provenance. 
1. The database will be preserved in perpetuity, housed initially at the New Mexico Interstate Stream
...
the Advancement of Hydrologic Science (CUAHSI). 
    a. We will also identify appropriate archiving institutions that might serve as a mirror
repository. 
    b. A data policy and stewardship plan will be established. In
